...
description: "The opcache extension mis-executes the ZENDTYPECHECK opcode in at least PHP 8.0.25 and causes a vital PHP expression, isresource($x), to curiously yield…"
sidebar:
  order: 130
---
...
We find that the Zend engine treats the PHP `is_resource(...)` function call specially (*[1] <https://github.com/php/php-src/blob/master/Zend/zend_compile.c#L4497>_*):
...
and that Zend compiles it to a `ZEND_TYPE_CHECK` opcode with extended_value being `(1 << IS_RESOURCE)` ([\[2\]](https://github.com/php/php-src/blob/master/Zend/zend_compile.c#L3945..L3950)).
...
When the two lines shown in the first block are removed, the `is_resource($x)` expression would instead be compiled to a `ZEND_INIT_FCALL` opcode ([\[3\]](https://github.com/php/php-src/blob/master/Zend/zend_compile.c#L4612)) and execution would eventually land in the C function corresponding to `is_resource` ([\[4\]](https://github.com/php/php-src/blob/php-8.0.25/ext/standard/type.c#L240..L276)).
...
There is a... peculiar comment in php-opcache (`MAY_BE_RESOURCE` is the same as `1 << IS_RESOURCE`) ([\[5\]](https://github.com/php/php-src/blob/master/ext/opcache/jit/zend_jit.c#L3515)) that could(?) be relevant:

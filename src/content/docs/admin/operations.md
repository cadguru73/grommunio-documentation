...
If following the grommunio Setup path, also see [TLS configuration](/admin/installation/#tls-configuration). Throughout the installation process, the administrator has multiple choices for TLS-based installation. For seamless operation, it is recommended to have a basic understanding of PKI concepts and the X.509 standard for certificates. Generally, grommunio uses PEM-encoded certificates.
...
With the snapshot mechanism provided by the storage provider, snapshots can be easily used to backup and restore entire mailboxes in a matter of seconds. For restoring mailboxes to another mailbox's identity, it is recommended to ensure the mailbox is not in active use (such as mobile devices, profile synchronization). After the restore operation has completed, it is advised to restart the services `gromox-http`, `gromox-midb` and `gromox-zcore` to invalidate any existing runtime caches:

``` bash
# systemctl restart gromox-http gromox-midb gromox-zcore
```

:::tip[Step-by-step procedures]
The [Backup and restore guide](/guides/backup-restore/) turns the artefact list below into a runbook: building a checksummed logical backup set, a full disaster-recovery rehearsal onto a fresh appliance, and restoring a single mailbox into the same or a different mailbox with the Gromox mailbox transfer tools.
:::
...
- the individual mailbox container is the `maildir` of each user below those trees (`grommunio-admin user query username maildir` prints the path)
- MySQL database: `grommunio`

2.  grommunio Files:

- `/var/lib/grommunio-files`
- `/usr/share/grommunio-files/config/config.php` (instance ID, secret and database credentials — required to re-attach a restored database)
- MySQL database: `grofiles`

3.  grommunio Chat:

- `/var/lib/grommunio-chat`
- `/etc/grommunio-chat/config.json` (database credentials and SSO secrets); the `chatID` columns of the `grommunio` database must be restored together with `grochat`
- MySQL database: `grochat`

4.  grommunio Archive:

- `/var/lib/grommunio-archive`
- `/etc/grommunio-archive` (configuration and, above all, the message encryption key `grommunio-archive.key` — without it the store cannot be read), `/etc/sphinx/sphinx.conf`, and the Postfix archive routing files `/etc/postfix/transport` and `/etc/postfix/grommunio-archiver-envelope.cf`
- MySQL database: `groarchive`

5.  grommunio Office:

- `/etc/grommunio-office/default.json` (Document Server configuration including the database credentials)
- MySQL database: `groffice`

6.  grommunio-auth / Keycloak (if single sign-on is used, see the [SSO guide](/guides/sso-keycloak/)):

- `/etc/grommunio-keycloak/` (`keycloak.conf`, `grommunio.properties`)
- `/etc/gromox/keycloak.json` and `/etc/gromox/bearer_pubkey`
- MySQL database: `grommunio_keycloak`

7.  grommunio Appliance:
...
- File backup of `/etc/grommunio-antispam` (Rspamd configuration), `/var/lib/grommunio-antispam` (DKIM private keys, local maps) and the Redis snapshot of the `redis@grommunio` instance (`dump.rdb` under the `dir` set in `/etc/redis/grommunio.conf`, by default `/var/lib/redis/default/`), which holds the learned Bayes statistics, greylisting and rate-limit state; also the DKIM keys generated by the Admin UI in `/var/lib/grommunio-admin-api/*.dkim.key*`. See the [Antispam guide](/guides/antispam/).
...
Backup the grommunio databases `grommunio`, `grofiles`, `groffice`, `groarchive` and `grochat` using standard procedures. Most backup solutions provide MySQL database backup agents for easy integration. For detailed backup options of your MySQL databases, refer to: <https://dev.mysql.com/doc/refman/8.0/en/backup-types.html>. If in doubt, the built-in utility `mysqldump` (<https://dev.mysql.com/doc/refman/8.0/en/mysqldump.html>) can create single SQL backup files of databases. A manual MySQL backup dump can be issued with:
...
- `HKEY_CURRENT_USER\Software\Microsoft\Office\<version>\Outlook\PST` `MaxLargeFileSize` (DWORD(32-bit), Decimal: 102400)

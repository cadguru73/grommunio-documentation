...
description: "Subscription customers can generate a support package by executing the command grommunio-support and send the created support package to grommunio's sup…"
sidebar:
  order: 90
---
...
Subscription customers can generate a support package by executing the command `grommunio-support` and send the created support package to grommunio's support for analysis to [support@grommunio.com](mailto:support@grommunio.com?subject=%5Bgrommunio%5D%20support%20request%3A%20TOPIC&body=License%3A%0D%0A%0D%0ASteps%20to%20reproduce%3A%0D%0A%0D%0AActual%20result%3A%0D%0A%0D%0AExpected%20result%3A%0D%0A%0D%0A---%0D%0A%0D%0AIf%20supplied%20with%20grommunio-support%20archive%20the%20following%20information%20is%20optional%3A%0D%0A%0D%0AEnvironment%20(Platform%2FOS)%3A%0D%0A%0D%0Agrommunio%20version%3A%0D%0A%0D%0AComponent%20(if%20known)%3A%0D%0A%0D%0ALogs%20(if%20applicable)%3A%0D%0A%0D%0A---%0D%0A%0D%0ACustom%20notes%3A%0D%0A%0D%0A---%0D%0A%0D%0AContact%20information%3A).

The archive generated is made available under the web root of grommunio admin archive, which is why it is strongly recommended to remove the generated support archive as soon as it has been transmitted to grommunio support. The support archive can be removed by accessing the console and executing the command `rm -f /usr/share/grommunio-admin-web/grommunio-support.txz`.
...
nginx writes one access/error log pair per location; the file names follow the pattern `/var/log/nginx/nginx-<service>-access.log` and `nginx-<service>-error.log`:

| URI Prefix | Process | Files |
|----|----|----|
| `/web` | nginx | `/var/log/nginx/nginx-web-access.log`, `/var/log/nginx/nginx-web-error.log` |
| `/web` | php-fpm | grommunio Web logs through PHP; see the `php-fpm` journal (`journalctl -u php-fpm`) |
| `/Microsoft-Server-ActiveSync` | nginx | `/var/log/nginx/nginx-sync-access.log`, `/var/log/nginx/nginx-sync-error.log` |
| `/Microsoft-Server-ActiveSync` | php-fpm | `/var/log/grommunio-sync/grommunio-sync.log`, `/var/log/grommunio-sync/grommunio-sync-error.log` |
| `/dav` | nginx | `/var/log/nginx/nginx-dav-access.log`, `/var/log/nginx/nginx-dav-error.log` |
| `/dav` | php-fpm | `/var/log/grommunio-dav/dav.log` |
| `/Autodiscover`, `/autodiscover` | nginx | `/var/log/nginx/nginx-autodiscover-access.log`, `nginx-autodiscover-error.log` |
| `/EWS` | nginx | `/var/log/nginx/nginx-ews-access.log`, `nginx-ews-error.log` |
| `/mapi`, `/rpc`, `/OAB` | nginx | `nginx-mapi-*.log`, `nginx-rpc-*.log`, `nginx-oab-*.log` |
| Admin UI (port 8443) | nginx | `/var/log/nginx/nginx-admin-access.log`, `nginx-admin-error.log` |
| `/chat`, `/meet`, `/files`, `/office`, `/archive`, `/auth` | nginx | `nginx-chat-*.log`, `nginx-meet-*.log`, `nginx-files-*.log`, `nginx-office-*.log`, `nginx-archive-*.log`, `nginx-auth-*.log` |
| `/chat` | grommunio-chat | `/var/log/grommunio-chat/mattermost.log`, `/var/log/grommunio-chat/notifications.log` |
| `/files` | php-fpm | `/var/log/grommunio-files/files.log` |
